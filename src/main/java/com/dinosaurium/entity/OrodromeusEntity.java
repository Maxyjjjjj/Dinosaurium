...
public class OrodromeusEntity extends Animal implements GeoEntity {
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(TEXTURE, "orodromeus");
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		OrodromeusEntity retval = DinosauriumModEntities.ORODROMEUS.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(DinosauriumModBlocks.EPHEDRA.get().asItem(), DinosauriumModBlocks.HORSETAIL.get().asItem()).contains(stack.getItem());
	}
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(DinosauriumModEntities.ORODROMEUS.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.orodromeus.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.orodromeus.idle"));
		}
